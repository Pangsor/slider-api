...
router.post('/cart', async(req, res) => {
  ...
  // Validasi No JO
  var sqlQuery = `SELECT no_job_order,kode_barang,kode_jenis_bahan FROM tt_po_job_order WHERE no_job_order = '${dataJson[1].no_job_order}' AND asal_divisi = '${dataJson[1].asal_divisi}' AND tujuan_divisi = '${dataJson[1].asal_divisi}' AND status_job = 'PROSES'`;
  var ResultSelect = await dbToko.SelectQuery(strIdCluster[1], sqlQuery)
    .then(result => { return [200, result[0]] }).catch(err => { return [500, err]});
  if (ResultSelect[0] === 500) {
    await dbToko.RollBackTransaction(strIdCluster[1]);
  ...
  for (i in detailBahan[1]){
    ...
    if (detailBahan[1][i].jumlah_bahan < 0){
      return res.send(`Jumlah bahan kurang atau sama dengan nol!`)
    }
    ...
    if (detailBahan[1][i].berat_bahan < 0){
      return res.send(`Berat bahan kurang atau sama dengan nol!`)
    }
  ...
  // Validasi Cart
  var sqlQuery = `SELECT no_job_order FROM tt_admin_kirim_detail_cart WHERE divisi = '${dataJson[1].nama_divisi}'
  ...
  return res.send({
    "status":"berhasil",
    "pesan":`Tambah produksi gudang qc berhasil disimpan.`,
    "data":[{}]
  });
})
...
router.post('/check-out', async(req, res) => {
  ...
  // Simpan Card
  const resCard = await insertCardAdmin(strIdCluster[1],id,tmpTgl,dataJson[1].nama_divisi,"PRODUKSI GUDANG QC","PRODUKSI GUDANG QC",dataJson[1].no_job_order,tmpKdJnsBahan,tmpBeratOut,"-","OUT");
  if  (resCard[0] !== 200){
    await dbToko.RollBackTransaction(strIdCluster[1]);
    dbToko.closeConnection(strIdCluster[1]);
  ...
  // Update Saldo
  const resSaldo = await saldoAdminKredit(strIdCluster[1],tmpTgl,dataJson[1].nama_divisi,tmpKdJnsBahan,tmpBeratOut,"-","berat_gudang_qc");
  if  (resSaldo[0] !== 200){
    await dbToko.RollBackTransaction(strIdCluster[1]);
    dbToko.closeConnection(strIdCluster[1]);
    return res.status(500).send("Internal Server Error !");
  ...
  // Update Status
  sqlQuery = `UPDATE tt_po_job_order SET status_job ='CLOS',tujuan_divisi='MARKETING',asal_divisi='MARKETING' WHERE no_job_order = '${dataJson[1].no_job_order}' LIMIT 1`;
...
router.post('/tgl/all', async(req, res) => {
  ...
  sqlQuery = `SELECT b.*,a.tgl_kirim FROM tt_admin_kirim_head a INNER JOIN tt_admin_kirim_detail b ON a.no_transaksi = b.no_transaksi
             WHERE (tgl_kirim BETWEEN '${dataJson[1].tgl_awal}' AND '${dataJson[1].tgl_akhir}')`;
  try{
    const resultSelect = await dbToko.SelectQuery(strIdCluster[1], sqlQuery)
      .then(result => { return [200, result[0]] }).catch(err => { return [500, err]});
    if (resultSelect[0] === 500) {
