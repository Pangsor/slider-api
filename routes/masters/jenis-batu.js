...
router.get('/all', async(req, res) => {
  ...
  try{
    const sqlQuery = `SELECT kode_jenis_batu,nama_jenis_batu FROM tm_jenis_batu`;
    ...
    if (resultSelect[1].length == 0){
      return res.send({
        "status":"berhasil",
        "pesan":"Data jenis batu tidak ditemukan",
        "data":resultSelect[1]
      });
    }else{
      return res.send({
...
router.post('/', async(req, res) => {
  ...
  const {error} = validateJenisBatuAdd(req.body);
  ...
  if (Object.keys(ResultSelect[1]).length !== 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode jenis batu : ${dataJson[1].kode_jenis_batu} sudah ada !`,
      "data":[{}]
    });
  }

  sqlQuery = `INSERT INTO tm_jenis_batu (kode_jenis_batu,nama_jenis_batu,input_by,input_date) VALUES 
    ('${dataJson[1].kode_jenis_batu}','${dataJson[1].nama_jenis_batu}','-','${tmpTgl}')`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data jenis batu berhasil disimpan.",
    "data":[{}]
  });
})
...
router.put('/', async(req, res) => {
  const {error} = validateJenisBatuEdit(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode jenis batu : ${dataJson[1].kode_jenis_batu} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `UPDATE tm_jenis_batu SET nama_jenis_batu = '${dataJson[1].nama_jenis_batu}' WHERE kode_jenis_batu = '${dataJson[1].kode_jenis_batu}'`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data jenis batu berhasil diedit.",
    "data":[{}]
  });
})
...
router.delete('/', async(req, res) => {
  const {error} = validateJenisBatuDelete(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Data jenis batu : ${dataJson[1].kode_jenis_batu} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `DELETE FROM tm_jenis_batu WHERE kode_jenis_batu = '${dataJson[1].kode_jenis_batu}' LIMIT 1`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data jenis batu berhasil dihapus.",
    "data":[{}]
  });
})

// ===============================================================================validasi
function validateJenisBatuAdd(bahan){
    const schema = Joi.object({
    kode_jenis_batu: Joi.string().min(1).max(30).required(),
    nama_jenis_batu: Joi.string().min(1).max(100).required()
  });
  return schema.validate(bahan);
}

function validateJenisBatuEdit(bahan){
  const schema = Joi.object({
    kode_jenis_batu: Joi.string().min(1).max(30).required(),
    nama_jenis_batu: Joi.string().min(1).max(100).required()
  });
...
function validateJenisBatuDelete(bahan){
  const schema = Joi.object({
    kode_jenis_batu: Joi.string().min(1).max(30).required()
  });
  return schema.validate(bahan);
