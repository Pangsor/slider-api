...
router.get('/all', async(req, res) => {
  ...
  try{
    const sqlQuery = `SELECT kode_jenis_bahan,nama_jenis_bahan,kode_warna FROM tm_jenis_bahan`;
    ...
    if (resultSelect[1].length == 0){
      return res.send({
        "status":"berhasil",
        "pesan":"Data jenis bahan tidak ditemukan",
        "data":resultSelect[1]
      });
    }else{
      return res.send({
...
router.post('/', async(req, res) => {
  ...
  const {error} = validateJenisBahanAdd(req.body);
  ...
  sqlQuery = `INSERT INTO tm_jenis_bahan (kode_jenis_bahan,nama_jenis_bahan,kode_warna,input_by,input_date) VALUES
             ('${dataJson[1].kode_jenis_bahan}','${dataJson[1].nama_jenis_bahan}','${dataJson[1].kode_warna}','-','${tmpTgl}')`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data jenis bahan berhasil disimpan.",
    "data":[{}]
  });
})
...
router.put('/', async(req, res) => {
  const {error} = validateJenisBahanEdit(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode jenis bahan : ${dataJson[1].kode_jenis_bahan} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `UPDATE tm_jenis_bahan SET nama_jenis_bahan = '${dataJson[1].nama_jenis_bahan}',kode_warna = '${dataJson[1].kode_warna}'
             WHERE kode_jenis_bahan = '${dataJson[1].kode_jenis_bahan}'`
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data jenis bahan berhasil diedit.",
    "data":[{}]
  });
})
...
router.delete('/', async(req, res) => {
  const {error} = validateJenisBahanDelete(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Data jenis bahan : ${dataJson[1].kode_jenis_bahan} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `DELETE FROM tm_jenis_bahan WHERE kode_jenis_bahan = '${dataJson[1].kode_jenis_bahan}' LIMIT 1`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data jenis bahan berhasil dihapus.",
    "data":[{}]
  });
})

// ===============================================================================Validasi
function validateJenisBahanAdd(bahan){
    const schema = Joi.object({
    kode_jenis_bahan: Joi.string().min(1).max(30).required(),
    nama_jenis_bahan: Joi.string().min(1).max(100).required(),
    kode_warna: Joi.string().min(1).max(30).required()
  });
  return schema.validate(bahan);
}

function validateJenisBahanEdit(bahan){
  const schema = Joi.object({
    kode_jenis_bahan: Joi.string().min(1).max(30).required(),
    nama_jenis_bahan: Joi.string().min(1).max(100).required(),
    kode_warna: Joi.string().min(1).max(30).required()
...
function validateJenisBahanDelete(bahan){
  const schema = Joi.object({
    kode_jenis_bahan: Joi.string().min(1).max(30).required()
  });
  return schema.validate(bahan);
