...
router.get('/all', async(req, res) => {
  ...
  try{
    const sqlQuery = `SELECT kode_marketing,nama_marketing,no_hp,email FROM tm_marketing`;
    ...
    if (resultSelect[1].length == 0){
      return res.send({
        "status":"berhasil",
        "pesan":"Data marketing tidak ditemukan",
        "data":resultSelect[1]
      });
    }else{
      return res.send({
...
router.post('/', async(req, res) => {
  ...
  const {error} = validateMarketingAdd(req.body);
  ...
  if (Object.keys(ResultSelect[1]).length !== 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode marketing : ${dataJson[1].kode_marketing} sudah ada !`,
      "data":[{}]
    });
  }

  sqlQuery = `INSERT INTO tm_marketing (kode_marketing,nama_marketing,no_hp,email,input_by,input_date) VALUES
             ('${dataJson[1].kode_marketing}','${dataJson[1].nama_marketing}','${dataJson[1].no_hp}','${dataJson[1].email}','-','${tmpTgl}')`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data marketing berhasil disimpan.",
    "data":[{}]
  });
})
...
router.put('/', async(req, res) => {
  const {error} = validateMarketingEdit(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode marketing : ${dataJson[1].kode_marketing} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `UPDATE tm_marketing SET nama_marketing = '${dataJson[1].nama_marketing}',no_hp = '${dataJson[1].no_hp}'
            ,email = '${dataJson[1].email}' WHERE kode_marketing = '${dataJson[1].kode_marketing}'`
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data marketing berhasil diedit.",
    "data":[{}]
  });
})
...
router.delete('/', async(req, res) => {
  const {error} = validateMarketingDelete(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Data marketing : ${dataJson[1].kode_marketing} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `DELETE FROM tm_marketing WHERE kode_marketing = '${dataJson[1].kode_marketing}' LIMIT 1`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data marketing berhasil dihapus.",
    "data":[{}]
  });
})

// ===============================================================================Validasi
function validateMarketingAdd(bahan){
    const schema = Joi.object({
    kode_marketing: Joi.string().min(1).max(30).required(),
    nama_marketing: Joi.string().min(1).max(100).required(),
...
function validateMarketingEdit(bahan){
  const schema = Joi.object({
    kode_marketing: Joi.string().min(1).max(30).required(),
    nama_marketing: Joi.string().min(1).max(100).required(),
    no_hp: Joi.string().min(1).max(50).required(),
...
function validateMarketingDelete(bahan){
  const schema = Joi.object({
    kode_marketing: Joi.string().min(1).max(30).required()
  });
  return schema.validate(bahan);
