...
router.get('/all', async(req, res) => {
  ...
  try{
    const sqlQuery = `SELECT kode_design,nama_design FROM tm_design`;
    ...
    if (resultSelect[1].length == 0){
      return res.send({
        "status":"berhasil",
        "pesan":"Data design tidak ditemukan",
        "data":resultSelect[1]
      });
    }else{
      return res.send({
...
router.post('/', async(req, res) => {
  ...
  const {error} = validateDesignAdd(req.body);
  ...
  if (Object.keys(ResultSelect[1]).length !== 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode design : ${dataJson[1].kode_design} sudah ada !`,
      "data":[{}]
    });
  }

  sqlQuery = `INSERT INTO tm_design (kode_design,nama_design,input_by,input_date) VALUES 
    ('${dataJson[1].kode_design}','${dataJson[1].nama_design}','-','${tmpTgl}')`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data design berhasil disimpan.",
    "data":[{}]
  });
})
...
router.put('/', async(req, res) => {
  const {error} = validateDesignEdit(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode design : ${dataJson[1].kode_design} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `UPDATE tm_design SET nama_design = '${dataJson[1].nama_design}' WHERE kode_design = '${dataJson[1].kode_design}'`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data design berhasil diedit.",
    "data":[{}]
  });
})
...
router.delete('/', async(req, res) => {
  const {error} = validateDesignDelete(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  sqlQuery = `DELETE FROM tm_design WHERE kode_design = '${dataJson[1].kode_design}' LIMIT 1`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data design berhasil dihapus.",
    "data":[{}]
  });
})

// ===============================================================================validasi
function validateDesignAdd(bahan){
    const schema = Joi.object({
    kode_design: Joi.string().min(1).max(30).required(),
    nama_design: Joi.string().min(1).max(100).required()
  });
  return schema.validate(bahan);
}

function validateDesignEdit(bahan){
  const schema = Joi.object({
    kode_design: Joi.string().min(1).max(30).required(),
    nama_design: Joi.string().min(1).max(100).required()
  });
...
function validateDesignDelete(bahan){
  const schema = Joi.object({
    kode_design: Joi.string().min(1).max(30).required()
  });
  return schema.validate(bahan);
