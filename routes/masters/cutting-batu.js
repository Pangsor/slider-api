...
router.get('/all', async(req, res) => {
  ...
  try{
    const sqlQuery = `SELECT kode_cutting_batu,nama_cutting_batu FROM tm_cutting_batu`;
    ...
    if (resultSelect[1].length == 0){
      return res.send({
        "status":"berhasil",
        "pesan":"Data cutting batu tidak ditemukan",
        "data":resultSelect[1]
      });
    }else{
      return res.send({
...
router.post('/', async(req, res) => {
  ...
  const {error} = validateCuttingAdd(req.body);
  ...
  if (Object.keys(ResultSelect[1]).length !== 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode cutting : ${dataJson[1].kode_cutting_batu} sudah ada !`,
      "data":[{}]
    });
  }

  sqlQuery = `INSERT INTO tm_cutting_batu (kode_cutting_batu,nama_cutting_batu,input_by,input_date) VALUES 
    ('${dataJson[1].kode_cutting_batu}','${dataJson[1].nama_cutting_batu}','-','${tmpTgl}')`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data cutting berhasil disimpan2.",
    "data":[{}]
  });
})
...
router.put('/', async(req, res) => {
  const {error} = validateCuttingEdit(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode cutting : ${dataJson[1].kode_cutting_batu} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `UPDATE tm_cutting_batu SET nama_cutting_batu = '${dataJson[1].nama_cutting_batu}' WHERE kode_cutting_batu = '${dataJson[1].kode_cutting_batu}'`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data cutting berhasil diedit.",
    "data":[{}]
  });
})
...
router.delete('/', async(req, res) => {
  const {error} = validateCuttingDelete(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Data cutting : ${dataJson[1].kode_cutting_batu} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `DELETE FROM tm_cutting_batu WHERE kode_cutting_batu = '${dataJson[1].kode_cutting_batu}' LIMIT 1`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data cutting berhasil dihapus.",
    "data":[{}]
  });
})

// ===============================================================================validasi
function validateCuttingAdd(bahan){
    const schema = Joi.object({
    kode_cutting_batu: Joi.string().min(1).max(30).required(),
    nama_cutting_batu: Joi.string().min(1).max(100).required()
  });
  return schema.validate(bahan);
}

function validateCuttingEdit(bahan){
  const schema = Joi.object({
    kode_cutting_batu: Joi.string().min(1).max(30).required(),
    nama_cutting_batu: Joi.string().min(1).max(100).required()
  });
...
function validateCuttingDelete(bahan){
  const schema = Joi.object({
    kode_cutting_batu: Joi.string().min(1).max(30).required()
  });
  return schema.validate(bahan);
