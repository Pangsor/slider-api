...
router.get('/all', async(req, res) => {
  ...
  try{
    const sqlQuery = `SELECT kode_batu,ukuran,nama_batu,kode_jenis_batu,kode_cutting_batu,berat_batu FROM tm_batu`;
    ...
    if (resultSelect[1].length == 0){
      return res.send({
        "status":"berhasil",
        "pesan":"Data batu tidak ditemukan",
        "data":resultSelect[1]
      });
    }else{
      return res.send({
...
router.post('/', async(req, res) => {
  ...
  const {error} = validateBatuAdd(req.body);
  ...
  if (Object.keys(ResultSelect[1]).length !== 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Kode batu : ${dataJson[1].kode_batu} sudah ada !`,
      "data":[{}]
    });
  }

  sqlQuery = `INSERT INTO tm_batu (kode_batu,ukuran,nama_batu,kode_jenis_batu,kode_cutting_batu,berat_batu,input_by,input_date) VALUES
             ('${dataJson[1].kode_batu}','${dataJson[1].ukuran}','${dataJson[1].nama_batu}','${dataJson[1].kode_jenis_batu}','${dataJson[1].kode_cutting_batu}'
             ,'${dataJson[1].berat_batu}','-','${tmpTgl}')`;
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data batu berhasil disimpan.",
    "data":[{}]
  });
})
...
router.put('/', async(req, res) => {
  ...
  sqlQuery = `UPDATE tm_batu SET ukuran = '${dataJson[1].ukuran}',nama_batu = '${dataJson[1].nama_batu}'
...
router.delete('/', async(req, res) => {
  const {error} = validateBatuDelete(req.body);
  if (error) return res.status(500).send({
    "status":"error",
    "pesan":error.details[0].message,
    "data":[{}]
  ...
  if (Object.keys(ResultSelect[1]).length === 0) {
    ...
    return res.status(500).send({
      "status":"error",
      "pesan":`Data batu : ${dataJson[1].kode_batu} tidak di temukan !`,
      "data":[{}]
    });
  }
  // End Cek Kode

  sqlQuery = `DELETE FROM tm_batu WHERE kode_batu = '${dataJson[1].kode_batu}' LIMIT 1`
  ...
  return res.send({
    "status":"berhasil",
    "pesan":"Data batu berhasil dihapus.",
    "data":[{}]
  });
})

// ===============================================================================Validasi
function validateBatuAdd(bahan){
    const schema = Joi.object({
    kode_batu: Joi.string().min(1).max(30).required(),
    ukuran: Joi.string().min(1).max(100).required(),
    nama_batu: Joi.string().min(1).max(100).required(),
...
function validateBatuDelete(bahan){
  const schema = Joi.object({
    kode_batu: Joi.string().min(1).max(30).required()
  });
  return schema.validate(bahan);
